...
private let kAnimationLayerKey = "com.MerrittWare.confetti.animationLayer"

/// A view that emits confetti.
public final class MxKConfettiView: UIView {
    public init() {
    ...
    public func emit(with contents: [Content],
                     for duration: TimeInterval = 3.0, completionBlock: (() -> Void)? = nil)
    {
        let layer = Layer()
...
extension MxKConfettiView: CAAnimationDelegate {
...
fileprivate extension MxKConfettiView.Content.Shape {
...
fileprivate extension MxKConfettiView.Content {
    var color: UIColor? {
        switch self {
